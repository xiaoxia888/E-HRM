...
def test_optional_entry_notice_uses_recorded_confirm_button(
    tmp_path: Path,
) -> None:
    settings = load_settings(Path("config/settings.toml"), data_root=tmp_path)
    page = Mock()
    ...
    button.click.side_effect = lambda **_kwargs: dialogs.count.configure_mock(
        return_value=0
    )
    ...
    assert button.click.call_args.kwargs["timeout"] > 0
    assert button.click.call_args.kwargs["force"] is True
    button.wait_for.assert_not_called()
    assert search_checks >= 2
    automation.progress_callback.assert_called_once_with(
        "退保：检测到停保时间提示，正在确认"
    )
...
def test_recorded_notice_and_entry_confirm_are_clicked_once_in_order(
    ...
    dialogs = Mock()
    dialogs.filter.return_value = dialogs
    dialogs.count.side_effect = lambda: 1 if state["notice"] else 0
    dialog = Mock()
    dialog.count.return_value = 1
    dialog.is_visible.return_value = True
    dialogs.nth.return_value = dialog
    ...
    dialog.locator.return_value = notice_buttons
    notice_button.click.side_effect = lambda **_kwargs: state.update(notice=False)

    entry_button = Mock()
    entry_confirms = Mock()
    entry_confirms.count.side_effect = (
        lambda: 1 if not state["notice"] and not state["entry"] else 0
    )
    entry_confirms.nth.return_value = entry_button
    entry_button.click.side_effect = lambda **_kwargs: state.update(entry=True)
    frame.get_by_role.return_value = entry_confirms
    ...
    automation._actionable = lambda locator: (
        state["entry"]
        if locator is search
        else locator is entry_button and not state["notice"] and not state["entry"]
    )

    automation._dismiss_entry_notice(frame)

    notice_button.click.assert_called_once()
    entry_button.click.assert_called_once()
